var debug = require('debug')('mongo_db');
var util = require('util');
var tool = require('leaptool');
var async = require('async');
var mongoDb = require('mongodb');

mongoDb.BSONPure = require('bson').BSONPure;
var ObjectID = mongoDb.BSONPure.ObjectID;

Database = function(app, callback) {
  this.app = app;
  this.host = app.setting.database.host;
  this.port = app.setting.database.port;
  this.dbname = app.setting.database.name || app.setting.database.dbname;
  this.username = app.setting.database.username;
  this.password = app.setting.database.password;
  // connect to mongo database
  var mongoServer = new mongoDb.Server(this.host, this.port, { auto_reconnect: true }, {});
  this.db = new mongoDb.Db(this.dbname, mongoServer, { safe: false });
  this.db.open(function(error, db) {
    if (error) {
      console.log("\nCan't connect to MongoDB @ " + this.host + ':' + this.port);
    } else {
      if (this.username && this.password) {
        this.db.authenticate(this.username, this.password, function(error, result) {
          console.log('db authenticate result:', error, result);
          callback && callback();
        }.bind(this));
      } else {
        callback && callback();
